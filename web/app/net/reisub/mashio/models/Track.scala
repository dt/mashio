...
object Track extends MetaModel[Track]("track") {
  ...
  def findByArtistAlbumAndTitle(artistId: Artist.Id, albumId: Album.Id, title: String) = 
    dao.findOne(Query("artist" -> artistId, "album" -> albumId, "title" -> title))
  
  def add(artist: Artist.Id, album: Album.Id, title: String, trackNum: Option[Int], length: Option[Int], path: Option[String],
