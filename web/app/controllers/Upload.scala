...
object Upload extends Controller {
  ...
  def handleMp3(request: play.api.mvc.Request[play.api.libs.Files.TemporaryFile]) = {
    ...
    try {
		...
		val album = Album.findOrCreateByArtistAndTitle(albumArtist._id, tag.getFirst(FieldKey.ALBUM), tag.getFirst(FieldKey.YEAR))
		...
		Track.findByArtistAlbumAndTitle(artist._id, album._id, title) match {
	      ...
	      case None => {
	        ...
	        Track.add(artist._id, album._id, title, trackNum, length, Some(fileName), id)
